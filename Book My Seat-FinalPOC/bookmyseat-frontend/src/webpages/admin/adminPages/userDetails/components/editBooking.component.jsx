...
export default function EditBooking({ userId}) {
  ...
  const handleFloorNameClick = (floorId, bookingDetails) => {
    ...
    axios
      .post("http://localhost:9006/bookmyseat/admin/seats", postData)
      .then((response) => {
        console.log("Floor ID posted successfully:", response.data);
      })
      .catch((error) => {
      
        console.error("Error posting floor ID:", error);
      });
  };
  ...
  const handleBookingCancellation = (bookingId) => {
    ...
    axios.put(`http://localhost:9006/bookmyseat/admin/cancel/${bookingId}`)
    .then((response) => {
